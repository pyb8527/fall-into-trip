...
const Ink = {
   ...
   * 바닥.
   *
   * <p>흰색이 아닙니다. 전에는 바닥도 흰색, 카드도 흰색이었고 그 둘을
   * 머리카락 굵기 선 하나가 갈랐습니다. 그러면 화면이 <b>테두리 쳐진 사각형의
   * 더미</b>가 됩니다 — 무엇이 무엇 위에 놓였는지는 선이 아니라 밝기가
   * 말해야 합니다. 한 단만 내려도 흰 카드가 종이 위에 놓인 것으로 읽힙니다.
   */
  25: '#FAFAFA',
  /** 눌러 앉은 자리(칸·타일) */
  50: '#F2F2F2',
 ...
 * 코랄 — 이 앱이 쓰는 단 하나의 색.
 *
 * <p>문토의 산호빛에서 가져왔습니다. 붉은 기가 돌지만 경고로 안 읽힐 만큼
 * 부드럽고, 무채색 화면에 한 점만 떨어뜨려도 눈이 거기로 갑니다.
 *
 * <p>세 단뿐입니다. 더 두면 어느 단을 쓸지가 매번 판단이 되고, 판단이 되면
 * 화면마다 달라집니다.
 */
const Coral = {
  /** 옅은 물. 고른 줄과 배지의 바탕. */
...
export const Colors = {
  ...
  /*
    ...
    바닥이 한 단 어둡고 그 위에 흰 카드가 놓입니다. 카드에 테두리를 두르지
  ...
  */
  ...
  /*
    ...
    검정 하나로 버티던 동안, 검정 단추가 셋 나란히 서면 어느 것이 주인지
    알 수 없었습니다. 색을 안 쓴 것이 아니라 <b>위계를 표시할 수단이 하나
    모자랐던</b> 것입니다.

    <h3>왜 셋인가</h3>

    한 가지 코랄로는 안 됩니다. 밝은 코랄에 흰 글씨를 얹으면 대비가
    3.91:1 로, WCAG AA 가 요구하는 4.5:1 에 못 미칩니다. 길 위에서 햇빛
    아래 보는 앱이라 이건 취향 문제가 아닙니다.

    그래서 자리마다 다른 단을 씁니다 — 옅은 물은 바탕에, 코랄은 표식과
    선택에, 진한 코랄은 <b>흰 글씨를 얹는 자리</b>에만.
   */
  /** 표식·선택·핀. 여기에는 흰 글씨를 얹지 않습니다. */
  accent: Coral.base,
  accentPressed: Coral.strong,
  /** 바탕에 까는 옅은 물. 고른 줄, 배지. */
  accentSoft: Coral.soft,
  accentSoftPressed: Coral.softPressed,
  accentText: Ink[0],
  /**
   * 흰 글씨를 얹는 자리. 주 단추가 이것입니다.
   *
   * <p>밝은 코랄보다 한 단 어둡습니다. 흰 글씨와 4.85:1 이라 AA 를 넘습니다.
   */
  accentStrong: Coral.strong,
  /** 강조를 글자·아이콘으로 쓸 때. 흰 바탕 위에 놓이므로 진한 쪽입니다. */
  accentInk: Coral.strong,
 ...
 *
 * <p>이제 면이 층을 이루고(바닥 한 단, 카드 한 단) 색이 하나 들어왔으므로
 * 모서리를 둥글려도 물러 보이지 않습니다.
 *
 * <h3>세 단만 둡니다</h3>
 *
