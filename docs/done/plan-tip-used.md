...
> `plan-news.md` 가 먼저 서야 합니다 — 같은 화면의 아래층입니다. 그것은
> 커밋 `39e2f14` 로 끝났습니다.
>
> **끝났습니다 — 2026-09-14.** 커밋 `86c290e` "남긴 한 줄이 쓰였다는 것을
> 남긴 사람에게 돌려준다". `V22__place_tip_views.sql` 을 씁니다.
>
> 본문은 **계획을 쓸 때의 글 그대로** 둡니다. 들어가기 전에 §1 의 전제
> 하나가 틀린 것을 찾아 §1.1 에 적었고, §8 의 순서를 하나 바꾼 이유는
> §10 에 있습니다.

## 1. 하려는 것

`PlaceTip`(한 줄)은 **남긴 사람에게 아무것도 돌려주지 않습니다.** 자기가
남긴 것이 쓰였는지 알 길이 없습니다. 남긴 사람에게 이 앱은 "한 번 글자를
넣은 곳" 으로 끝납니다.
...
> `PlaceTip` 은 이 앱에서 **여행이 끝난 뒤에도 계속 값이 생기는 유일한
> 것**입니다. "2번 출구로 나와야 함" 을 남기면 몇 달 뒤 다른 사람이 그
> 장소를 열 때 쓰입니다.

**그렇지 않습니다.** `TipService.FRESH` 가 7일이고, 목록 질의가
`createdAtAfter(now - FRESH)` 로 좁힙니다(`TipService.java:45,64`). 팁은
**일주일이면 안 보입니다.** 화면도 그렇게 적어 두었습니다 —
"최근 일주일 안에 다녀온 사람들이 남긴 것입니다"(`tip-sheet.tsx:100`).

7일인 데는 이유가 있습니다. "지금 대기 40분" 은 다음 날이면 이미 거짓말
입니다. 하지만 "2번 출구로 나와야 함" 은 안 낡습니다. **한 칸에 성격이
다른 둘이 들어 있고, 짧은 쪽에 맞춰 잘려 있습니다.**

이 계획은 그것을 안 건드립니다. 세는 일은 7일이든 몇 달이든 똑같이 맞고,
기한을 바꾸는 것은 팁이 무엇인지를 다시 정하는 일이라 여기서 혼자 정할
것이 아닙니다.

**대신 수가 작을 것입니다.** 한 줄이 일주일만 서 있으니 "47번" 이 아니라
"3번" 일 수 있습니다. 붙여 놓고 실제로 얼마가 나오는지 보는 것이 이
기한을 다시 볼 근거가 됩니다.
...
**`varchar(12)` 라고 적었던 것을 16 으로 고쳤습니다.** 이 저장소의 번호는
전부 `VARCHAR(16)` 입니다 — `place_tips.id`, `users.id`, `trip_post_views`
셋 다 그렇습니다. 12 로 만들면 `Ids.next()` 가 내는 값이 잘립니다.

`trip_post_views` 처럼 `REFERENCES ... ON DELETE CASCADE` 를 겁니다. 팁을
지우면 그 조회도 같이 지워져야 합니다 — 없는 팁의 조회수가 남아 있으면
합계가 실제보다 큽니다.
...
- ~~**"47명" 인지 "47번" 인지.**~~ **정했습니다 — "번" 입니다.**
  열쇠가 `(tipId, userId, onDate)` 라 같은 사람이 다른 날 보면 둘로
  셉니다. 그러면 "명" 이 아닙니다. `count(*)` 로 세고 화면에 "번" 이라고
  적습니다.

  사람 수로 바꾸려면 `count(distinct user_id)` 인데, 그럴 이유가
  없습니다. §1 이 보여 주려는 것은 "실제로 남을 도운 **횟수**" 이고,
  같은 사람이 다음 주에 그 가게를 다시 찾아보며 또 읽었다면 그것도 한
  번 쓰인 것입니다. 그리고 "명" 이라 적어 놓고 사람-날짜를 세면 아는
  것과 다른 말을 하는 것이 됩니다.
...
| 어디 | 무엇 |
|---|---|
| `V22__place_tip_views.sql` | 표 하나. `(tip_id, user_id, on_date)` 가 열쇠 |
| `tip/domain/PlaceTipView.java` | `PostView` 와 같은 모양 |
| `tip/domain/PlaceTipViewRepository.java` | 하루 한 번인지 묻는 것 하나 |
| `tip/application/TipService.countViews` | 세는 자리 |
| `tip/api/TipController.list` | 목록을 준 **뒤에** 셉니다 |
| `news/application/MineRow.java` | 합계 모양 |
| `news/application/NewsFeed.mine` | 합계 질의 |
| `news/application/NewsService` | `View` 에 `mine` 한 덩이 |
| `api/types.ts` · `app/(app)/news.tsx` | 아래 요약 줄 |

### §8 의 순서를 하나 바꿨습니다

§8 은 "3 이 초록불이 되기 전에 4 로 가지 않습니다" 였습니다. **그럴 수가
없습니다** — 세는 것을 볼 창이 `/api/news` 의 요약뿐이고, 그것이 4 단계에
생깁니다. 팁마다 조회수를 띄우지 않기로 했으니(§3.1) 다른 창이 없습니다.

그래서 3 과 4 를 같이 세우고, **시험은 3 의 규칙만 봅니다.** 창이 4 일 뿐
재는 것은 세는 규칙입니다.

### 읽는 일과 세는 일을 나눴습니다

`listOf` 가 `@Transactional(readOnly = true)` 입니다. 거기에 쓰기를 넣으면
**손님이 목록을 볼 때마다 쓰기 거래가 열립니다.** `countViews` 를 따로 두고
컨트롤러가 차례로 부릅니다.
...
| | 왜 |
|---|---|
| 손님 | 사람 번호가 없어 "하루 한 번" 을 셀 수 없습니다. 아이피로 세면 자취를 쌓는 일입니다 |
| 내 팁 | 장소를 열 때마다 내 수가 오릅니다 |
| 같은 날 두 번째 | 열쇠 셋이 막습니다. 세는 쪽에 조건문이 필요 없습니다 |
| 내려간 팁 | 목록이 이미 거르지만, 부르는 쪽이 바뀌어도 안 새도록 `countViews` 가 한 번 더 봅니다 |

### 합계는 `LEFT JOIN` 입니다

아직 아무도 안 읽은 팁도 **남긴 수**에는 들어가야 합니다. 안쪽 조인이면
"남긴 한 줄 4개" 가 읽힌 것만 세어 2개가 됩니다.

### 화면이 부풀리지 않습니다

```
남긴 한 줄 4개가 7번 쓰였습니다.
로그인하고 본 것만 셉니다. 실제로는 더 쓰였을 수 있습니다.
```

둘째 줄이 있어야 합니다. 손님이 읽은 것은 셀 수가 없는데 그것을 안 밝히면,
이 수 하나 때문에 나머지 화면까지 못 믿게 됩니다.

읽은 사람이 아직 없으면 `남긴 한 줄 4개. 아직 읽은 사람이 없습니다.` 로
적습니다. 한 줄도 안 남긴 사람에게는 **줄 자체가 안 섭니다**(§6.2).

### 잰 것

`tip.test.mjs` **42 통과 / 0 실패** (26 에서 열여섯 늘었습니다). §7 이
적은 여덟을 다 담고 여덟이 늘었습니다.

| 무엇을 | |
|---|---|
| 아직 아무도 안 읽음 | 0 에서 시작 |
| 남이 읽으면 는다 | |
| 같은 사람이 다시 읽어도 안 는다 | 세 번 읽혀도 1 |
| 내가 내 것을 읽으면 안 는다 | |
| 손님이 읽으면 안 는다 | |
| 다른 사람이 읽으면 는다 | 2 |
| 남긴 수가 하나 는다 | |
| 내려가면 남긴 수에서 빠진다 | |
| 내려간 것은 조회도 안 센다 | |
| 한 줄도 안 남기면 요약이 없다 | `mine` 이 `null` |
| 남기면 요약이 생긴다 | |
| **남의 조회수가 안 섞인다** | §7 이 가장 중요하다고 한 것 |
| **내 것만 는다** | 같음 |

같이 돌린 것 — news 54 · community 54 · comment 24 · admin 56 · trip 60 ·
guest 34, 전부 0 실패. `./gradlew test` 27 통과, typecheck 과 web export
통과.

### 아직 안 잰 것

**수가 실제로 얼마나 나오는지.** §1.1 이 적은 대로 팁은 7일이면 안 보입니다.
"47번" 이 아니라 "3번" 일 수 있습니다. 붙여 놓고 보는 것이 그 기한을 다시
볼 근거가 됩니다.

### 곁에 남겨 둔 것

§9 의 나머지 둘 그대로입니다 — "내가 남긴 것" 화면을 따로 둘지는 요약 줄을
눌러 보는 사람이 있는지 보고, 오래된 조회를 버릴지는 `PostView` 와 같이
생각해야 할 문제라 여기서 혼자 정하지 않습니다.
