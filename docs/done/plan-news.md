...
> 끝났습니다.
>
> **끝났습니다 — 2026-09-11.** 커밋 셋입니다.
>
> | | 무엇 |
> |---|---|
> | `39e2f14` | 본체. `V21__news_seen.sql`, `news` 묶음, `app/(app)/news.tsx` |
> | `a8fe6bd` | 한 곳에서 온 것을 한 줄로 접기 (§11) |
> | `080d931` | 표에 끝난 것을 적기 |
>
> 본문(§1~§9)은 **계획을 쓸 때의 글 그대로** 둡니다. 들어가면서 §3 에
> 둘(3.6·3.7)이 늘었고, 올린 뒤에 하나(3.9)를 더 고쳤습니다 — §10·§11 에
> 무엇이 계획과 달랐는지 적혀 있습니다.
...
## 11. 접습니다 (2026-09-11, 올린 당일 고침)

### 3.9 한 곳에서 온 것은 한 줄

올리고 나서 물어본 것입니다 — **"추천한 사람이 다 뜨면 엄청 많지 않을까?"**

그랬습니다. 추천 한 건이 소식 한 줄이었습니다. 글 하나가 좀 받은 날
서른 줄이 전부 "추천했습니다" 가 되고, 동행자가 고친 일정은 그 아래로
밀려납니다. 소식함은 **내가 없는 동안 무엇이 바뀌었나**를 보는 자리이지
인기를 세는 자리가 아닙니다.

**§3.4 가 이미 같은 판단을 내려 두었습니다.**

> `'오사카 3박 4일' 을 12명이 봤어요` 를 12줄로 올리면 목록이 그것만으로
> 찹니다.

조회수에는 그 규칙을 쓰고 추천·댓글에는 안 썼습니다. 같은 고장입니다.

### 무엇을 접었나

| 갈래 | 전 | 후 |
|---|---|---|
| `post.like` | 추천 하나에 한 줄 | 글마다 한 줄 |
| `post.comment` | 댓글 하나에 한 줄 | 글마다 한 줄 |
| `candidate.vote` | 표 하나에 한 줄 | 후보마다 한 줄 |
| `place.add`·`place.edit`·`candidate.add` | 하나씩 | 그대로 |

장소와 후보는 접을 것이 없습니다. 벌어진 일이 아니라 **지금 있는 줄**을
읽으므로 애초에 하나씩입니다. 표가 위험한 것은 동행자 다섯에 후보 서른이면
백쉰 개까지 간다는 점입니다 — 글만큼은 아니어도 목록을 먹기에는 충분합니다.

### 한 사람일 때는 이름을 붙입니다

접으면서 "다 님이" 를 잃고 싶지 않았습니다.

| | 문장 |
|---|---|
| 한 사람 | `다 님이 「오사카 3박 4일」 를 추천했습니다.` |
| 여럿 | `「오사카 3박 4일」 를 13명이 추천했습니다.` |
| 한 사람의 표 | `나 님이 「우메다 공중정원」 에 좋다고 했습니다.` |
| 여럿의 표 | `「우메다 공중정원」 에 3명이 답했습니다.` |

표가 여럿이면 좋다·아니라를 안 가릅니다. 갈렸을 수 있고, 어느 쪽인지는
투표장이 이미 보여 줍니다.

`GROUP BY` 하나로 세면서 한 사람일 때의 이름까지 얻으려고
`min(v.userId)` 을 씁니다. **한 사람일 때 그것이 그 한 사람입니다.**
여럿이면 이름을 안 붙이므로 누가 뽑히든 상관없습니다.

### 세는 일은 데이터베이스가 합니다

줄을 서른 개만 읽어 와서 자바에서 세는 길도 있었습니다. 그러면 추천이
마흔이어도 "30명" 이라고 말합니다. **아는 것보다 적게 말하는 것도
틀린 말입니다.** `count(...)` 로 셉니다.

### 응답이 달라진 곳

`actorName` 이 **비어 있을 수 있습니다.** 여럿이 접힌 줄이고, 그때는 몇
사람인지가 `text` 안에 있습니다. 화면은 이름이 있을 때만 "님이" 를 붙입니다.

`named()` 가 비어 있는 자리를 "누군가" 로 채우지 않게 했습니다. 채우면
열세 사람이 한 사람으로 보입니다.

### 잰 것

`news.test.mjs` **54 통과 / 0 실패** (45 에서 아홉 늘었습니다).

새로 들어간 [8-1] 이 열세 사람에게 추천과 댓글을 시킵니다. 스물여섯 건이
**두 줄**로 접히고, 여행 쪽 소식 셋이 그대로 남고, 목록 전체가 다섯 줄에
머뭅니다. 접지 않았으면 스물아홉 줄이었고 그중 스물여섯이 게시판이었습니다.

같이 돌린 것 — community 54 · comment 24 · copy 57 · candidate 25 ·
admin 56 · trip 60, 전부 0 실패. `./gradlew test` 27 통과, typecheck 과
web export 통과.
