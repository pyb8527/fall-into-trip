...
## 11. 만들었습니다 — 재는 사람을 바꿔서 (2026-09-14)

§8 의 1번은 구글 키를 요구하고, 키는 **운영에만** 있습니다. 그래서 게이트를
없애지 않고 **재는 사람을 바꿨습니다** — 만들어서 올리고, 키가 있는 데서
사람이 재고, 다르지 않으면 그때 걷어냅니다. `plan-booking-paste.md` 와 같은
방식입니다.

**이 문서의 게이트는 살아 있습니다.** §9 첫 줄("별 차이가 없으면 안 합니다")은
아직 답을 못 받았습니다. 여기 적힌 것은 "됐다" 가 아니라 "잴 수 있게 됐다"
입니다.
...
| 어디 | 무엇 |
|---|---|
| `PlaceSearchService.search(…, polyline)` | 갈래 하나. 본문에 `searchAlongRouteParameters` |
| `RouteService.pathBetween(from, to, mode)` | 구간 폴리라인 하나. 못 구하면 `null` |
| `RecommendService.Between` | 번호 둘 |
| `RecommendService.endsOf` | **이 여행의 것인지 확인** |
| `RecommendService.noteFor` | 되돌아갔으면 그렇게 말합니다 — 11.2 |
| `TripController.RecommendRequest.between` | 양쪽 엔드포인트 |
| `recommend-sheet.tsx` | "어디까지 가는 길인가요?" 줄 |

### 11.1 캐시를 따로 안 봅니다

§3.3 이 "있으면 재사용, 없으면 부름" 을 어떻게 짤지 고민했는데, §10.1 이
답을 준 뒤로는 **`leg()` 를 그냥 부르면 됩니다.** 그것이 이미 봅니다.

문턱도 저절로 맞습니다(§10.2). 캐시에 맞으면 `ask()` 를 안 지나가 아무것도
안 쓰고, 없으면 `ask()` 가 1 을 씁니다. `GoogleQuotaFilter` 는 한 줄도 안
고쳤습니다.

### 11.2 §6 을 하나 어겼습니다 — 응답의 `note`

§6 은 "응답 안 바뀝니다" 였습니다. `note` 한 줄이 늘었습니다.

§4 의 되돌아가기 때문입니다. 길을 못 구하거나 결과가 비면 조용히 언저리
검색으로 내려앉는데, **조용하면 잴 수가 없습니다.** 사람이 칩을 껐다 켜며
답이 달라지는지 보는 것이 이번 측정인데, "달라지지 않음" 과 "되돌아감" 이
똑같이 보입니다.

그리고 재는 일과 무관하게도 맞습니다. 물은 것과 답한 것이 다르면 말해야
합니다 — 집 규칙의 "아는 것만 말합니다" 가 그것입니다.

> 가는 길을 못 구해서 언저리에서 찾았습니다. 두 곳이 아주 가까우면 그렇습니다.

### 11.3 화면 — §9 가 미뤄 둔 것

"구간 표시 옆" 과 "추천 시트 안" 중 **시트 안**으로 정했습니다.

시트에 이미 "어디 근처에서 찾을까요?" 가 있습니다. 거기에 도착지를 하나
얹으면 물음이 달라집니다 — "오사카성 근처 점심" 이 "오사카성에서 도톤보리
가는 길에 점심" 이 됩니다. 새 자리를 만드는 것보다 **이미 있는 물음을 한 칸
늘리는** 쪽이 배울 것이 적습니다.

기준이 '지금 내 자리' 나 잘 곳이면 이 줄이 **안 뜹니다.** 서버가 이 여행의
장소 번호 둘을 받는데 그 둘에는 번호가 없습니다.

### 11.4 잰 것 — 넷은 여기서, 하나는 거기서

§7 의 다섯 중 **넷이 키 없이 섭니다.** 키가 없으면 검색이 "꺼져 있습니다"
로 끝나는데, 그 말이 나왔다는 것은 **그 앞의 울타리를 전부 지났다**는
뜻입니다 — 거절은 전부 검색보다 먼저 섭니다.

| §7 | 무엇 | |
|---|---|---|
| 1 | `between` 을 주면 결과가 오는가 | **키 필요** |
| 2 | `between` 없이는 지금과 같은가 | 잼 |
| 3 | 다른 여행의 것이면 400 | 잼 |
| 4 | 동행자가 아니면 404 | 잼 |
| 5 | **남의 여행 placeId 가 새지 않는가** | 잼 |

5번을 두 줄로 봅니다 — 남의 여행에 **실제로 있는** 번호와 **아예 없는**
번호가 **같은 말로** 거절되는지. 다르면 그것으로 남의 여행을 들여다볼 수
있습니다.

`recommend.test.mjs` **30 통과 / 0 실패** (21 에서 아홉 늘었습니다).

같이 돌린 것 — place 9 · trip 60 · icon 42 · quota 7 · news 54 · tip 42,
전부 0 실패. `./gradlew test` 27 통과, typecheck 과 web export 통과.

### 11.5 잡은 것 하나

여행 없는 `/api/recommend` 가 `between` 을 **조용히 버리고** 있었습니다.
그 자리 주석이 날짜에 대해 이미 규칙을 적어 두었습니다 — "여기서 조용히
버리면 왜 그날 기준으로 안 보나 를 알 길이 없습니다. 서비스가 거절합니다."
같은 규칙을 `between` 에도 씁니다.

### 11.6 형님이 재 볼 것

운영에서, 같은 날 두 곳을 고르고 **"어디까지 가는 길인가요?" 를 껐다 켜며**
같은 문장을 물어봅니다.

| 본 것 | 뜻 |
|---|---|
| 답이 **달라진다** | 이 계획이 값을 합니다. 그대로 둡니다 |
| 답이 **같다** | §9 첫 줄대로 **걷어냅니다** |
| "가는 길을 못 구해서…" 가 뜬다 | 두 곳이 너무 가깝습니다. 더 먼 두 곳으로 다시 |

멀리 떨어진 두 곳으로 재는 편이 낫습니다. §9 가 적어 둔 대로 같은 날 연달아
있는 두 곳은 대개 가까워서 결과가 빌 수 있습니다.
...
| 물음 | |
|---|---|
| 구글이 실제로 다르게 답하는지 | **그대로.** 11.6 |
| 요금 등급을 정말 안 올리는지 | **그대로.** 청구 내역으로만 |
| 어느 구간을 기본으로 삼을지 | **기본을 안 두기로 했습니다.** 사람이 고릅니다 — 무엇을 고를지는 묻는 사람만 압니다 |
| ~~화면에서 어떻게 열지~~ | **정했습니다 — 11.3** |
